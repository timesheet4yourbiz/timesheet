...
export async function loadSidebar() {
    ...
    // --- RBAC: AMBIL DATA PENGGUNA TERLEBIH DAHULU ---
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    const email = session.user.email;
    const { data: emp } = await supabase.from('employees').select('name, role').eq('email', email).single();
    
    // Kenal pasti status admin (True / False)
    const isAdmin = emp && emp.role === 'admin';
    const nameStr = (emp && emp.name) ? emp.name : email.split('@')[0];
    
    // Sembunyikan menu 'EMPLOYEES' sepenuhnya dari kod HTML jika BUKAN Admin
    ...
    const sidebarHTML = `
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <!-- MENU EMPLOYEES HANYA KELUAR JIKA ADMIN -->
                ${employeesMenuHTML}
    ...
    // --- KUNCI HALAMAN (RBAC Peringkat Kedua) ---
    // Simpan status isAdmin dalam objek global `window` supaya fail lain (projects.js, dll) boleh periksa
