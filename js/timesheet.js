        ...
        const loadTimesheetData = async () => {
            ...
            document.querySelectorAll('.time-input').forEach(input => {
                ...
                input.addEventListener('change', async (e) => {
                    ...
                    else if (!rawVal.includes(':') && !rawVal.includes('.')) rawVal = rawVal + ':00'; // Sekarang ia faham perpuluhan!
                    ...
                    // Kalau pengguna klik tapi tak ubah nilai, jangan bazir memori
                    if (el.value === el.getAttribute('data-oldval')) return;
                    el.setAttribute('data-oldval', el.value);

                    el.style.opacity = '0.5'; 
                    // Simpan senyap-senyap ke database
                    await saveTimeEntry(dateStr, pid, tid, totalSeconds, false);
                    el.style.opacity = '1'; 
                    
                    // KIRA JUMLAH LOKAL SAHAJA (Fokus dijamin kekal di kotak!)
